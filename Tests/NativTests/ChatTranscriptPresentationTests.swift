...
final class ChatTranscriptPresentationTests: XCTestCase {
    ...
    func testResearchFallbackIsPresentedAsIncompleteInsteadOfFailed() throws {
        let context = ChatActivityContext(id: UUID(), kind: .deepResearch)
        let final = ChatTranscriptMessage(
            role: .assistant,
            content: "Research collected 3 sources, but this model did not finish the written synthesis.",
            activityContext: context
        )
        ...
        XCTAssertEqual(activity.status, .incomplete)
    }
    ...
    func testConversationExportIncludesModelAndRecordedMetrics() {
        ...
        XCTAssertTrue(export.contains("Models: mlx-community/Test-Model"))
        XCTAssertTrue(export.contains("Recorded usage: 1200 total tokens · 200 generated"))
        XCTAssertTrue(export.contains("Metrics: 1200 total tokens · 200 generated"))
        XCTAssertTrue(export.contains("42.5 tok/s"))
        XCTAssertTrue(export.contains("8.25 GB peak memory"))
        XCTAssertTrue(export.contains("75% draft acceptance"))
        XCTAssertTrue(export.contains("Thinking: 1.5 s"))
    }
