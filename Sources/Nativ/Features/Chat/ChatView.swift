    ...
    private func runChatLoop(_ queuedRequest: QueuedChatRequest) async throws {
        ...
        while true {
               ...
               ChatResearchFinalizationPolicy.needsRetry(
                ...
                guard finalization.retryCount == 0 else {
                    failAssistantMessage(
                        assistantMessageID,
                        in: queuedRequest.sessionID,
                        error: ChatResearchFinalizationError.noFinalAnswer
                    )
                    return
                }
            ...
            if let cachedResults = researchController?.cachedResults(for: toolCalls) {
                ...
                researchFinalization = ChatResearchFinalizationState(reason: .repeatedCalls)
                assistantMessageID = UUID()
            ...
            if var controller = researchController {
                if let reason = controller.record(researchObservations) {
                    advertisesTools = false
                    researchFinalization = ChatResearchFinalizationState(reason: reason)
                }
...
private struct ChatAgentStepCell: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            if isAwaitingImageModelSelection {
                ...
                if let webActivity, !webActivity.sources.isEmpty {
                    Divider()
                        .padding(.top, 7)
                    ChatWebSourceStrip(sources: webActivity.sources)
                        .padding(.top, 7)
                }
                if isExpanded {
                    Divider()
    ...
    private var details: some View {
        if let webActivity {
            ChatWebActivityDetails(activity: webActivity)
                .padding(.top, 7)
        } else {
            VStack(alignment: .leading, spacing: 8) {
    ...
    private var webActivity: ChatWebActivity? {
        ChatWebSourcePresentation.activity(for: message)
    }

    private var activityLabel: String? {
        guard let webActivity else { return nil }
        return webActivity.query ?? webActivity.focus
    }
